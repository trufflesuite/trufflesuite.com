...
1. Download ("unbox") the MetaCoin box:
   ...
   <p class="alert alert-info">
   <strong>Note</strong>: You can use an optional `--force` to initialize the project in the current directory regardless of its state (e.g. even if it contains other files or directories). Be careful, this will potentially overwrite files that exist in the directory. This applies to be both the `init` and `unbox` commands.
   </p>
