...
<p class="alert alert-info">
<strong>Note</strong>: We're using `web3.eth.getAccounts()` in these examples, which returns a promise which resolves to an array of all the accounts generated by the mnemonic. So, given the addresses generated by our mnemonic above, specifying `web3.eth.getAccounts().then(accounts => accounts[0])` is equivalent to the address `0x627306090abab3a6e1400e9345bc60c78a8bef57`.
</p>
...
* See how much ether that balance is worth (and note that the contract defines a metacoin to be worth 2 ether):
  ...
  ```shell
  MetaCoin.deployed().then(instance => web3.eth.getAccounts().then(accounts => instance.getBalanceInEth(accounts[0]).then(value => value.toNumber())));
  ```

* Transfer some metacoin from one account to another:

  ```shell
  MetaCoin.deployed().then(instance => web3.eth.getAccounts().then(accounts => instance.sendCoin(accounts[1], 500)));
  ```

* Check the balance of the account that *received* the metacoin:

  ```shell
  MetaCoin.deployed().then(instance => web3.eth.getAccounts().then(accounts => instance.getBalance(accounts[1]).then(value => value.toNumber())));
  ```
